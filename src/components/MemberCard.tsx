...
export default function MemberCard({ member, index, isCurrentUser, onDelete }: MemberCardProps) {
    ...
    return (
        <Tilt
            ...
            gyroscope={false}
            className={`${isLeader ? 'col-span-2 lg:col-span-4 lg:w-1/3 lg:mx-auto' : ''}`}
        >
            <div
                className={`member-card gaming-card rounded-[1.5rem] md:rounded-[2rem] overflow-hidden relative flex flex-col h-auto ${isLeader ? 'md:h-[550px]' : 'md:h-[420px] lg:h-[480px] group'}`}
                data-aos="zoom-in"
